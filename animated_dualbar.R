...
for (state in states){
  ...
  dual_bar <- ggplot(data=state_df) +
    #geom_bar(data=state_df, aes(x=factor(year2), y=jobs, fill=forcats::fct_rev(factor(scenario)), colour=scenario), stat="identity") +
    ...
    geom_bar(stat="identity", position=position_dodge(), aes(x=factor(year), y=jobs, fill=forcats::fct_rev(factor(scenario)))) + 
    scale_fill_manual(values=c(alpha("#37b3e5", .5), "#1f1446"))+
  ...
  ggsave("C:/tmp/ITC/charts/USA_jobs_hires.png", width=19.20, height=10.8, dpi=1000)
