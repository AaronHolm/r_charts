...
seia_style <- function() {
       ...
       ggplot2::theme(
              ...
              axis.title.y = ggplot2::element_text(family=font, size=20, color="#1f1446", face="bold", margin=margin(0,10, 0, 0)), axis.title.x = ggplot2::element_blank(), 
...
nonsmi = dbGetQuery(con, "select * from products.eia_plus where sheet = 'Capacity - Current Installs'")

#nonsmi <- read_csv("C:/tmp/eia_plus.csv")
nonsmi_states <- c("Alaska", "Kansas", "Nebraska", "North Dakota", "South Dakota", "West Virginia", "Maine")
nonsmi <- nonsmi %>%
          filter(sheet=='Capacity - Current Installs' & state_full %in% nonsmi_states)
nonsmi <- nonsmi %>%
          group_by(state_full, sector, year) %>%
          summarise(value=sum(value))
less1_states <- c("Alaska", "North Dakota", "South Dakota", "West Virginia")

for(state in less1_states){
  state_annual <- nonsmi %>% filter(state_full==state)
  state_chart <- ggplot(state_annual, aes(x=factor(year), y=value, fill=forcats::fct_rev(factor(sector, levels=c("Residential", "Non-Residential", "Utility"))))) + geom_bar(stat='identity', width=0.5) + labs(y="Capacity (MW)", x="", title=paste(state,"Annual Solar Installations")) + seia_style() + scale_fill_manual(values=c("#37b3e5", "#ffe14f", "#2f70af")) + scale_y_continuous(expand=expand_scale(mult=c(0,0.02)), label=function(x)sprintf("%.2f", x)) + scale_x_discrete(labels=c("2019"="2019 Q1")) + guides(fill=guide_legend(reverse=TRUE))
  #ggsave(paste("C:/tmp/factsheets/charts/web/",state,".png", sep=""), width=18.72, height=7.2, dpi=100)
  state_chart
}
state_chart
#write_csv(nonsmi, "C:/tmp/nonsmi_test.csv")
nonsmi
